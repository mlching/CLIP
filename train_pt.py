...
def train(dataset: ClipCocoDataset, val_dataset: ClipCocoDataset, model: ClipCaptionModel, args,
    ...
    with open(f"{output_dir}/{output_prefix}.csv", 'w', newline='') as f:
        writer = csv.writer(f)
        writer.writerow(['Epoch', 'Train_loss', 'Val_loss', 'bleu_score', 'meteor_score'])
    train = []
    val = []
    b4 = []
    meteor = []
    for epoch in range(epochs):
        ...
        val_progress = tqdm(total=len(val_dataloader), desc="Validation")
        training_loss = 0.0
        ...
        with torch.no_grad():
            ...
            for idx, (val_tokens, val_mask, val_prefix) in enumerate(val_dataloader):
                ...
                val_loss = nnf.cross_entropy(val_logits.reshape(-1, val_logits.shape[-1]), val_tokens.flatten(), ignore_index=0)
                val_progress.set_postfix({"validation loss": val_loss.item()})
                val_progress.update()
        ...
        with open(f"{output_dir}/{output_prefix}.csv", 'a', newline='') as f:
            writer = csv.writer(f)
            writer.writerow([epoch, training_loss, validation_loss, bleu_score, meteor_score])
...
def main():
    ...
    val_dataset = ClipCocoDataset('./data/val_data4.pt', prefix_length, normalize_prefix=args.normalize_prefix)
